...
object fp3:
  ...
  def member(a: Int, xs: List[Int]): Boolean =
    ...
    xs match
      ...
      case x :: xs => if(a == x) then return true else member(a, xs)
  ...
  def allEqual(xs: List[Int]): Boolean =
    ...
    xs match
      ...
      case x :: xs => if(x == xs.head) then allEqual(xs.tail) else return false
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ...
    ys match
      case Nil => Nil
      case y::ys => if(y == x) then delete1(y, ys) else y::delete1(x, ys)
  ...
  def removeDupes1[X](xs: List[X]): List[X] =
    ...
    xs match
      case Nil => Nil
      case x::Nil => List(x)
      case x::xs => if(x == xs.head) then removeDupes1(xs) else x::removeDupes1(xs)
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    ...
    def rd2 [x] (x:X, xs:List[X],i:Int) : List[(Int, X)] = 
      xs match 
        case Nil => if(x == Nil) then List() else (i, x)::List()
        case y::ys => if(y == x) then rd2(x,ys,i+1) else (i,x)::rd2(y,ys,1)
    if(xs.length > 0) then rd2(xs.head, xs.tail, 1) else List()
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    ...
    (n, xs) match 
      case(_, Nil) => (Nil, Nil)
      case(-1, xs) => (List(), xs)
      case (n, x::xs) => {val(lista, listb) = splitAt(n-1, xs) 
