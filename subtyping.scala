...
object subtyping{
  ...
  {   
    class fCounter extends Counter {
    private var a = 0;
    override def decrement () = {a = a + 1}
    override def increment () = {a = a + 1 }
    override def get () : Int = a
    }
    val x = new fCounter();
    f(x);
    x.get();
  ...
  def observeCounterList(f: List[Counter] => Unit): List[Int] =
    // TODO: Provide definition here.
  {
    class ObserveCounter extends Counter {
    private var a = 0;
    override def decrement () = {a = a + 1}
    override def increment () = {a = a + 1 }
    override def get () : Int = a
    ...
    val l = new ObserveCounter();
    val m = new ObserveCounter();
    val n = new ObserveCounter();
    f(List(l, m, n));
    list = List(l.get(), m.get(), n.get());
    list;
  }
    //throw UnsupportedOperationException()
  ...
  def observeCounterArray(f: Array[Counter] => Unit): Array[Int] =
    // TODO: Provide definition here.
  {
    class observe extends Counter {
    private var a = 0;
    override def decrement () = {a = a + 1}
    override def increment () = {a = a + 1 }
    override def get () : Int = a
    }
    var carray : Array[Int] = new Array[Int] (3);
    val l = new observe();
    val m = new observe();
    val n = new observe();
    f(Array(l, m, n));
    carray = Array(l.get(), m.get(), n.get());
    carray;
  }
}
    //throw UnsupportedOperationException()
